...
//    }
    ...
    private val _signupUiState = MutableStateFlow(SignupOrganizationUiState())
    val signupUiState = _signupUiState

    fun updateUserSignupInput(userInput: SignupOrganizationUiState) {
        ...
        viewModelScope.launch {
            try{
                val signupData = SignupModel(
                    ...
                    email = signupUiState.value.emailAddress,
                )
                val signupResponse = ApiClient.retrofitService.signup(signupData)
                Log.d("signupResponse", "Signup response data: $signupResponse")
            } catch (e: Exception) {
